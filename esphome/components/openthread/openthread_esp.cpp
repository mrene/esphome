...
void OpenThreadComponent::ot_main() {
...
#ifndef CONFIG_OPENTHREAD_FORCE_DATASET
  ...
  if (dataset.mLength == 0) {
    ...
    size_t len = (sizeof(CONFIG_OPENTHREAD_TLVS) - 1) / 2;
    if (len > sizeof(dataset.mTlvs)) {
      ESP_LOGW(TAG, "TLV buffer too small, truncating");
      len = sizeof(dataset.mTlvs);
    }
    int n = parse_hex(CONFIG_OPENTHREAD_TLVS, sizeof(CONFIG_OPENTHREAD_TLVS) - 1, dataset.mTlvs, len);
    dataset.mLength = len;
  }
#endif
